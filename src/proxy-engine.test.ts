...
describe('ProxyEngine', () => {
  ...
  it('should forward requests and emit log events when capturing', async () => {
    const engine = new ProxyEngine({ targetHost: 'localhost', targetPort, proxyPort: 0 });
    const logs: any[] = [];

    engine.on('log', (result) => logs.push(result));
        ...
        (res: IncomingMessage) => {
          ...
          res.on('end', () => {
            try {
              ...
              expect(logs[0].request.path).toBe('/v1/chat/completions');
              expect(logs[0].response.status_code).toBe(200);
              expect(logs[0].response.prompt_tokens).toBe(10);
              resolve();
  ...
  it('should not emit log events when not capturing', async () => {
    const engine = new ProxyEngine({ targetHost: 'localhost', targetPort, proxyPort: 0 });
    const logs: any[] = [];
